...
export default class NavigationMapbox extends React.Component {
  ...
  componentDidMount = () => {
    ...
    if (this.props.navCallFor == "destination") {
      if (Object.keys(this.props.waypoints).length > 0) {
        this.setState({
          firstWaypoint: [
            this.props.waypoints[0].stop_lng,
            this.props.waypoints[0].stop_lat,
          ],
        });
      }
      if (Object.keys(this.props.waypoints).length > 1) {
        this.setState({
          secondWaypoint: [
            this.props.waypoints[1].stop_lng,
            this.props.waypoints[1].stop_lat,
          ],
        });
      }
    }
  ...
  locationChange = (event) => {
    ...
    //   geocollection.doc(value).update({
            ...
            Object.keys(origin).length > 0 ? (
              <>
                <MapboxNavigation
                  ...
                  destination={[destination.longitude, destination.latitude]}
                  firstWaypoint={
                    firstWaypoint.length > 0 ? firstWaypoint : null
                  }
                  secondWaypoint={
                    secondWaypoint.length > 0 ? secondWaypoint : null
                  }
                  shouldSimulateRoute={false}
                  showsEndOfRouteFeedback={false}
                  onLocationChange={(event) => this.locationChange(event)}
                  onRouteProgressChange={(event) =>
                    this._routeProgressChange(event)
                  }
                  onError={(event) => {
                    const { message } = event.nativeEvent;
                    console.error(
                      "mapbox navigation error================>",
                      message
                    );
                  }}
                  onCancelNavigation={() => {
