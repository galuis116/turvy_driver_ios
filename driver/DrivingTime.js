...
export default class DrivingTime extends React.Component {
  ...
  async componentDidMount() {
    await fetch(DOMAIN + "api/drivingtimepolicy")
      .then((response) => response.json())
      .then((json) => {
        //console.log("SERVICES TYPE 1");
        console.log("drivingTimePolicy", json.data);
        this.setState({
          drivingTimePolicy: json.data,
        });
      })
      .catch((error) => {
        //AsyncStorage.setItem('servicetypes', json.data);
        console.error("drivingTimePolicy_error", error);
      });
          ...
          <View style={styles.content}>
            <View style={{ elevation: 2, borderRadius: 5, marginTop: 10 }}>
              ...
              <View style={{ flex: 1, flexDirection: "row", padding: 15 }}>
                ...
                <View style={{ width: 20 }}></View>
                <View style={{ flex: 1 }}>
                  <Text style={localStyle.timeTxt}>
                    {this.state.timeData.driving_time_text}
                  </Text>
                  <Text style={localStyle.lableTxt}>
            ...
            {this.state.drivingTimePolicy.length > 0 ? (
              <>
