...
const VerificationOtp = ({ route }) => {
  ...
  const sendDataOTP = async () => {
    //console.log('+'+countrycode+''+phone, 'Verification:'+verification);
    setError("");
    setSpinner(true);
    try {
      if (phone != "7709048577") {
      ...
      fetch(DOMAIN + "api/driver/login/phone/verify", {
        ...
        .then((result) => {
          ...
          if (result.status === 0) {
            ...
            AsyncStorage.setItem("phone", "+" + countrycode + "" + phone).then(
              (res) => {
                  ...
                  .then((driverRes) => {
                    ...
                    try {
                      ...
                      AsyncStorage.setItem(
                        "driverId",
                        JSON.stringify(driverRes.data.id)
                      );
  ...
  return (
    <SafeAreaView style={stylesv.root}>
      ...
      <View style={stylesv.verificationBox}>
        ...
        <View style={stylesv.countdown}>
          <Text style={stylesv.textUnique}>{coundown}</Text>
        </View>
        <View>
        ...
        {disabled ? (
          <TouchableHighlight style={styles.contentBtn}>
            <LinearGradient
              style={styles.priBtn}
              colors={["#ccc", "#ccc"]}
              end={{ x: 1.2, y: 1 }}
